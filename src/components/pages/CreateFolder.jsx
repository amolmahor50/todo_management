...
export default function CreateFolder() {
    ...
    const handleRightClick = (e, folderName) => {
        ...
        // Prevent selection for "All" and "Uncategorised" folders
        ...
        setRightClickedFolder((prev) => {
            ...
            return [...prev, folderName]; // ✅ Add folder to selection
        });
    ...
    const toggleFolderSelection = (folderName) => {
        ...
        setRightClickedFolder((prevSelected) => {
            if (prevSelected.includes(folderName)) {
                return prevSelected.filter(name => name !== folderName); // ✅ Remove folder if already selected
            } else {
                return [...prevSelected, folderName]; // ✅ Add folder to selection
            }
        });
    };
            ...
            {isContextMenuOpen ? (
                <>
                    <div className="flex justify-between items-center">
                        ...
                        <VscChecklist className="sm:text-2xl text-xl cursor-pointer" />
                    </div>
                    <div className="grid gap-2 mt-6">
                        {Array.isArray(folderName) &&
                            folderName.map((folder, index) => {
                                ...
                                const isExcluded = folder.name === "All" || folder.name === "Uncategorised"; // ✅ Exclude condition
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {!isExcluded ? ( // ✅ Hide checkbox for "All" and "Uncategorised"
                                            isSelected ? (
                        ...
                        <div
                            onClick={() => {
                                if (rightClickedFolder.length === 1) {
                                    setInputFolderName(rightClickedFolder[0]); // Set input value to selected folder name
                                    setOpenCreateFolderPopUp(true); // Open the pop-up
                                }
                            }}
